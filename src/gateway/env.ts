...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  const isZAIGateway = normalizedBaseUrl?.includes('api.z.ai');

  // AI Gateway vars take precedence
  // Map to the appropriate provider env var based on the gateway endpoint
  if (env.AI_GATEWAY_API_KEY) {
    if (isZAIGateway) {
      // z.ai has built-in OpenClaw provider: uses ZAI_API_KEY env var
  ...
  // z.ai: OPENAI_API_KEY is also accepted as ZAI_API_KEY for built-in zai provider
  if (!envVars.ZAI_API_KEY && env.OPENAI_API_KEY && isZAIGateway) {
    envVars.ZAI_API_KEY = env.OPENAI_API_KEY;
  }
  ...
  if (normalizedBaseUrl) {
    ...
    if (isZAIGateway) {
      // z.ai uses built-in provider, no base URL override needed
      // But pass it for start-moltbot.sh detection
    } else if (isOpenAIGateway) {
